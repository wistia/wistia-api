  ...
  module Deprecation #:nodoc:
          ...
          alias_method_chain(method_name, :deprecation) do |target, punctuation|
            class_eval(<<-EOS, __FILE__, __LINE__ + 1)
              def #{target}_with_deprecation#{punctuation}(*args, &block)          # def generate_secret_with_deprecation(*args, &block)
                ::WistiaActiveSupport::Deprecation.warn(                                 #   ::ActiveSupport::Deprecation.warn(
                  self.class.deprecated_method_warning(                            #     self.class.deprecated_method_warning(
                    :#{method_name},                                               #       :generate_secret,
                    #{options[method_name].inspect}),                              #       "You should use WistiaActiveSupport::SecureRandom.hex(64)"),
                  caller                                                           #     caller
                )                                                                  #   )
    ...
    class DeprecatedObjectProxy < DeprecationProxy
      ...
      private
        ...
        def warn(callstack, called, args)
          WistiaActiveSupport::Deprecation.warn(@message, callstack)
        end
    end
    ...
    class DeprecatedInstanceVariableProxy < DeprecationProxy #:nodoc:
      ...
      private
        ...
        def warn(callstack, called, args)
          WistiaActiveSupport::Deprecation.warn("#{@var} is deprecated! Call #{@method}.#{called} instead of #{@var}.#{called}. Args: #{args.inspect}", callstack)
        end
    end
    ...
    class DeprecatedConstantProxy < DeprecationProxy #:nodoc:
      ...
      private
        ...
        def warn(callstack, called, args)
          WistiaActiveSupport::Deprecation.warn("#{@old_const} is deprecated! Use #{@new_const} instead.", callstack)
        end
    end
  end
end
...
class Module
  include WistiaActiveSupport::Deprecation::ClassMethods
end
