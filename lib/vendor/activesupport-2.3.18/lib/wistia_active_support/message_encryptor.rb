...
module WistiaActiveSupport
  ...
  class MessageEncryptor
    ...
    def encrypt(value)
      ...
      [encrypted_data, iv].map {|v| WistiaActiveSupport::Base64.encode64s(v)}.join("--")
    end
    
    def decrypt(encrypted_message)
      cipher = new_cipher
      encrypted_data, iv = encrypted_message.split("--").map {|v| WistiaActiveSupport::Base64.decode64(v)}
